...
public class CsvQueryResultExporterImpl implements CsvQueryResultExporter {
	...
	try {
	    ...
	    throw new ServiceException("Encoding: "
		    + conf.encoding
		    + " caused error during export: "
		    + e.getMessage(), e);
	}
    }
	...
	ImmutableSet<Map.Entry<CqlColumnValue, CqlColumnValue>> displayMap = extractor
		.extractMap(row, column).entrySet();
