...
public class HistoryPanel extends Panel implements AjaxReloadSupport {
	...
	private String decorateQueryWithFilter(QueryEntry entry) {
		...
		if (filterKeywords != null) {
			query = StringHelper
					.decorate(query, KEYWORD_DECORATOR, filterKeywords.toArray(new String[filterKeywords.size()]));
		}
		return query;
	}
	...
	private final class FilterBehaviour extends OnChangeAjaxBehavior {
		...
		protected void onUpdate(AjaxRequestTarget target) {
			...
			if (filterValue != null) {
				...
				FilterResult<QueryEntry> filterResult = searchService
						.filter(historyToUpdate, FILTER_ACCESSOR, kwordsArr);
