...
public abstract class AbstractImporterCase extends AbstractTestCase {
	...
	try (InputStream fio = getClass().getResourceAsStream("/cql/createDemoData.cql")) {
	    ...
	    ImportStats stats = getImporter().importScript(
		    fio,
		    rc,
	...
	queryService.execute(new CqlQuery(
		CqlQueryType.SELECT,
	...
	assertEquals(
		"some text in title ;)",
		queryService.execute(testCql).iterator().next().getString("title"));
	...
	{
	    CqlQueryResult res = queryService.execute(new CqlQuery(
		    CqlQueryType.SELECT,
...

	{
		...
		queryService.execute(new CqlQuery(
			CqlQueryType.UPDATE,
			"UPDATE CqlDemo.MyCounter SET cval=cval-"
				+ foundCounterVal
				+ " WHERE id=44a2054c-f98b-43a7-833d-0e1358fdaa82"));
	    }
	}
