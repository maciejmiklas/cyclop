...
public class TestQueryService extends AbstractTestCase {
	...
	ImmutableSortedSet<CqlColumnName> resp = qs.findColumnNames(Optional.of(new CqlTable(
		"cqldemo",
		"MyBooks")));
	assertNotNull(resp);
	...
	CqlQuery query = new CqlQuery(
		CqlQueryType.SELECT,
		"select * from CompoundTest where deesc='TEST_SET_1'");
	...
	assertTrue(comColsStr, rowMetadata.columns.contains(new CqlExtendedColumnName(
		...
		CqlDataType.create(DataType.uuid()),
		"id")));

	assertTrue(comColsStr, rowMetadata.columns.contains(new CqlExtendedColumnName(
		CqlColumnType.CLUSTERING_KEY,
		CqlDataType.create(DataType.cint()),
		"id2")));

	assertTrue(comColsStr, rowMetadata.columns.contains(new CqlExtendedColumnName(
		CqlColumnType.CLUSTERING_KEY,
		CqlDataType.create(DataType.varchar()),
		"id3")));
	...
	qs.execute(new CqlQuery(CqlQueryType.USE, "USE CqlDemo"));
	CqlQueryResult res = qs.execute(new CqlQuery(
		CqlQueryType.SELECT,
		"select * from MyBooks where pages=2212"));

	CqlRowMetadata rowMetadata = res.rowMetadata;
	assertTrue(res.toString(), rowMetadata.columns.contains(new CqlExtendedColumnName(
		CqlColumnType.REGULAR,
		CqlDataType.create(DataType.varchar()),
		"title")));
	...
	assertTrue(comColsStr, rowMetadata.columns.contains(new CqlExtendedColumnName(
		CqlColumnType.REGULAR,
		CqlDataType.create(DataType.set(DataType.varchar())),
		"authors")));
	...
	assertTrue(comColsStr, rowMetadata.columns.contains(new CqlExtendedColumnName(
		CqlColumnType.REGULAR,
		CqlDataType.create(DataType.map(DataType.varchar(), DataType.cdouble())),
		"price")));
