...
(function ($) {
	 ...
	 * Make suggest:
	 * 
	 * create and return jQuery object on the top of DOM object and
	 * registerIdentifier suggests as part of this object
	 * 
	 * @param area: HTML DOM element to add suggests to @param suggests: The
	 * array of suggest strings @param options: The options object
	 */
	$.makeSuggest = function (area, options) {
		...
		$area.getSuggestChunkBeforeCursor = function () {
			var delimiters = $area.options.delimiters.split(''); // array of
			// chars
			...
			if ($area.lastSuggestActive) {
				...
				while (matchIdx >= $area.options.minChunkSize) {
					...
					if (textBeforeCursor.endsWith(endsWithText)) {
						suggestFromLast = $area.lastSuggest.substr(matchIdx,
							$area.lastSuggest.length);
						break;
					}
					matchIdx--;
				}
		...
		/**
		 * completion when user types text (inputs next characters into
		 * textarea)
		 */
		...
		$area.isInputUpperCase = function () {
			...
			return selectionText.toUpperCase() === selectionText
				&& selectionText.toLowerCase() !== selectionText;
		};

		/**
		 * @param completionText
		 *            completion text after cursor (the highlighted part)
		 */
