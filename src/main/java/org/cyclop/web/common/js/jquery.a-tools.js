...
window.onload = init;
jQuery.fn.extend({getSelection: function () {
    ...
    if (document.selection) {
        ...
        if (d.text) {
            ...
            if (typeof b.selectionStart == "number") {
                a = b.selectionStart;
                c = b.selectionEnd;
                if (a == c)return{start: a, end: c, text: d.text, length: c - a}
            } else {
                a = b.createTextRange();
                e = a.duplicate();
                firstRe = a.text;
                ...
                if (firstRe == secondRe && firstRe != d.text || e.text.length > firstRe.length)return{start: caretPositionAmp[c], end: caretPositionAmp[c], text: "", length: 0};
                a = e.text.length;
            ...
            a -= f;
            c -= i;
            return{start: a, end: c, text: d.text, length: c - a}
        }
        b.focus();
