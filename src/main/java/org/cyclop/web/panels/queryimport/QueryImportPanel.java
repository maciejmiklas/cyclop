...
public class QueryImportPanel extends Panel {
	...
	buf.append("Executed ").append(nf.format(stats.errorCount + stats.successCount))
		.append(" queries in ").append(stats.runtime.toString()).append(", ")
		.append(nf.format(stats.successCount)).append(" were succesfull, ")
		.append(nf.format(stats.errorCount)).append(" failed");
	...
	ImportStats stats = impoter.importScript(
		new ByteArrayInputStream(fileContentBytes),
		result,
		importOptions.isIncludeInHistory(),
